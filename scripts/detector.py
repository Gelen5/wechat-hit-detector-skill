...
def fmt_html(r):
    ...
    if p["predict"] is None and p.get("scenario_reads"):
        ...
        scene_body = f'<div class="scene-note"><b>假设粉丝规模阅读情景</b><span>按有效打开率计算，仅供横向参考，不代表账号实际阅读量</span><div class="scenario-grid">{scenario_cards}</div></div>'
        scene_meta = f'三档假设规模 · 有效打开率 {p["eff_open_rate"]}%'
    else:
        scene_body = '<div class="empty">提供粉丝数后显示账号情景阅读量。</div>'
        scene_meta = "需要账号粉丝数"
    dims = [(DIM_NAMES[k], k, DIM_FULL[k]) for k in ["title", "opening", "content", "structure", "topic", "readability", "visual", "interaction"]]
    ...
    if suggestions:
        suggestion_cards = "".join(f'<article class="suggestion"><small>{s["pri"]} · {esc(s["title"])}</small><h3>{esc(s["title"])}</h3><p>{esc(s["detail"])}</p></article>' for s in suggestions)
    ...
    audience_rows = "".join(f'<div class="score-row"><span>{esc(a["name"])}</span><div class="bar"><i style="width:{a["resonance"]}%"></i></div><b>{a["resonance"]}</b></div>' for a in aud)
    ...
    ai_risk = r["ai_smell"]["risk"]
    risk_body = f'<div class="success">✓ 写作风格风险：{ai_risk}/100<br>✓ 口语化、对话感和自然表达明显<br><small>风险分不代表 AI 来源概率。</small></div>{compliance}'
    css = """
